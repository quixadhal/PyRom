...
def do_mstat(ch, argument):
    ...
    ch.send("Lv: %d  Class: %s  Align: %d  Gold: %ld  Silver: %ld  Exp: %d\n" % (
        victim.level,
        "mobile" if merc.IS_NPC(victim) else victim.guild.name,
        victim.alignment, victim.gold, victim.silver, victim.exp ))
    ch.send("Armor: pierce: %d  bash: %d  slash: %d  magic: %d\n" % (
        merc.GET_AC(victim, merc.AC_PIERCE), merc.GET_AC(victim, merc.AC_BASH),
        merc.GET_AC(victim, merc.AC_SLASH), merc.GET_AC(victim, merc.AC_EXOTIC)))
    ...
    if merc.IS_NPC(victim) and victim.off_flags:
    ...
    if merc.IS_NPC(victim) and victim.spec_fun is not None:
