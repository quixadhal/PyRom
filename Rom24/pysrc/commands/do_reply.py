...
def do_reply(ch, argument):
    ...
    if not victim.desc and not victim.is_npc():
        act("$N seems to have misplaced $S link...try again later.", ch, None, victim, merc.TO_CHAR)
        buf = "%s tells you '%s'\n" % (state_checks.PERS(ch, victim), argument)
        victim.buffer.append(buf)
        return
    if not ch.is_immortal() and not state_checks.IS_AWAKE(victim):
        act("$E can't hear you.", ch, 0, victim, merc.TO_CHAR)
        return
    ...
    if (victim.comm.is_set(merc.COMM_QUIET) or victim.comm.is_set(merc.COMM_DEAF)) \
        ...
        act("$E is not receiving tells.", ch, None, victim, merc.TO_CHAR, merc.POS_DEAD)
        return
    ...
    if victim.comm.is_set(merc.COMM_AFK):
        if victim.is_npc():
            act("$E is AFK, and not receiving tells.", ch, None, victim, merc.TO_CHAR, merc.POS_DEAD)
            return
        act("$E is AFK, but your tell will go through when $E returns.", ch, None, victim, merc.TO_CHAR,
                         merc.POS_DEAD)
        buf = "%s tells you '%s'\n" % (state_checks.PERS(ch, victim), argument)
        victim.buffer.append(buf)
        return
    act("You tell $N '$t'", ch, argument, victim, merc.TO_CHAR, merc.POS_DEAD)
    act("$n tells you '$t'", ch, argument, victim, merc.TO_VICT, merc.POS_DEAD)
    victim.reply = ch
    return
