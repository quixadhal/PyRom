...
def do_sell(ch, argument):
    ...
    if not state_checks.IS_OBJ_STAT(obj, merc.ITEM_SELL_EXTRACT) and roll < ch.get_skill("haggle"):
    ...
    if keeper.silver < 0:
        keeper.silver = 0
    if obj.item_type == merc.ITEM_TRASH or state_checks.IS_OBJ_STAT(obj, merc.ITEM_SELL_EXTRACT):
        obj.extract()
    else:
        obj.from_char()
        if obj.timer:
