...
def do_give(ch, argument):
    ...
    if arg1.isdigit():
        ...
        if merc.IS_NPC(victim) and merc.IS_SET(victim.act, merc.ACT_IS_CHANGER):
            ...
            if change < 1 and victim.can_see(ch):
                merc.act("$n tells you 'I'm sorry, you did not give me enough to change.'", victim, None, ch,
                         merc.TO_VICT)
                ch.reply = victim
    ...
    if not victim:
        ch.send("They aren't here.\n")
        return
    if merc.IS_NPC(victim) and victim.pIndexData.pShop is not None:
    ...
    if victim.carry_number + obj.get_number() > victim.can_carry_n():
        ...
        return
    if merc.get_carry_weight(victim) + obj.get_weight() > victim.can_carry_w():
