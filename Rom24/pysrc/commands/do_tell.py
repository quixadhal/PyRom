...
def do_tell(ch, argument):
    ...
    if not victim or ( merc.IS_NPC(victim) and victim.in_room != ch.in_room ):
        ...
        return
    if victim.desc == None and not merc.IS_NPC(victim):
    ...
    if (merc.IS_SET(victim.comm, merc.COMM_QUIET) or merc.IS_SET(victim.comm, merc.COMM_DEAF)) and not merc.IS_IMMORTAL(ch):
