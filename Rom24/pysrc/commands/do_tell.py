...
def do_tell(ch, argument):
    ...
    if (victim.comm.is_set(merc.COMM_QUIET) or state_checks.IS_SET(victim.comm,
            ...
            ch):
        act("$E is not receiving tells.", ch, 0, victim, merc.TO_CHAR)
        return

    if victim.comm.is_set(merc.COMM_AFK):
        if victim.is_npc():
            act("$E is AFK, and not receiving tells.", ch, None, victim, merc.TO_CHAR)
            return
        act("$E is AFK, but your tell will go through when $E returns.", ch, None, victim, merc.TO_CHAR)
        buf = "%s tells you '%s'\n" % (state_checks.PERS(ch, victim), argument)
        victim.buffer.append(buf)
        return
    act("You tell $N '$t'", ch, argument, victim, merc.TO_CHAR)
