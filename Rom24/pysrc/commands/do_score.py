...
def do_score(ch, argument):
    ch.send("You are %s%s, level %d, %d years old (%d hours).\n" % (ch.name, "" if merc.IS_NPC(ch) else ch.pcdata.title,
            ch.level, ch.get_age(), (ch.played + (int)(time.time() - ch.logon)) // 3600))
    ...
    ch.send("Race: %s  Sex: %s  Class: %s\n" % (ch.race.name, "sexless" if ch.sex == 0 else "male" if ch.sex == 1 else "female",
              "mobile" if merc.IS_NPC(ch) else ch.guild.name))
    ...
    ch.send("You are carrying %d/%d items with weight %ld/%d pounds.\n" % (ch.carry_number, ch.can_carry_n(),
              merc.get_carry_weight(ch) // 10, ch.can_carry_w() // 10))
    ...
    if not merc.IS_NPC(ch) and ch.level < merc.LEVEL_HERO:
    ...
    if not merc.IS_NPC(ch) and ch.pcdata.condition[merc.COND_THIRST] == 0:
        ch.send("You are thirsty.\n")
    if not merc.IS_NPC(ch) and ch.pcdata.condition[merc.COND_HUNGER] == 0:
        ch.send("You are hungry.\n")

    if ch.position == merc.POS_DEAD: ch.send("You are DEAD!!\n")
    elif ch.position == merc.POS_MORTAL: ch.send("You are mortally wounded.\n")
    elif ch.position == merc.POS_INCAP: ch.send("You are incapacitated.\n")
    elif ch.position == merc.POS_STUNNED: ch.send("You are stunned.\n")
    elif ch.position == merc.POS_SLEEPING: ch.send("You are sleeping.\n")
    elif ch.position == merc.POS_RESTING: ch.send("You are resting.\n")
    elif ch.position == merc.POS_SITTING: ch.send("You are sitting.\n")
    elif ch.position == merc.POS_STANDING: ch.send("You are standing.\n")
    elif ch.position == merc.POS_FIGHTING: ch.send("You are fighting.\n")
    # print AC values */
    if ch.level >= 25:
        ch.send("Armor: pierce: %d  bash: %d  slash: %d  magic: %d\n" % (
                  merc.GET_AC(ch, merc.AC_PIERCE),
                  merc.GET_AC(ch, merc.AC_BASH),
                  merc.GET_AC(ch, merc.AC_SLASH),
                  merc.GET_AC(ch, merc.AC_EXOTIC)))
    for i in range(4):
        temp = ''
        if i == merc.AC_PIERCE: temp = "piercing"
        elif i == merc.AC_BASH: temp = "bashing"
        elif i == merc.AC_SLASH: temp = "slashing"
        elif i == merc.AC_EXOTIC: temp = "magic"
        else: temp = "error"
        ch.send("You are ")

        if merc.GET_AC(ch, i) >= 101: ch.send("hopelessly vulnerable to %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= 80: ch.send("defenseless against %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= 60: ch.send("barely protected from %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= 40: ch.send("slightly armored against %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= 20: ch.send("somewhat armored against %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= 0: ch.send("armored against %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= -20: ch.send("well-armored against %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= -40: ch.send("very well-armored against %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= -60: ch.send("heavily armored against %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= -80: ch.send("superbly armored against %s.\n" % temp)
        elif merc.GET_AC(ch, i) >= -100: ch.send("almost invulnerable to %s.\n" % temp)
    ...
    if ch.level >= 15:
        ch.send("Hitroll: %d  Damroll: %d.\n" % (merc.GET_HITROLL(ch), merc.GET_DAMROLL(ch)))
    if ch.level >= 10:
