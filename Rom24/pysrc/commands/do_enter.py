...
def do_enter(ch, argument):
    ...
    if argument:
        ...
        or (merc.IS_SET(portal.value[1],merc.EX_CLOSED) and not merc.IS_TRUSTED(ch,merc.ANGEL)):
            ch.send("You can't seem to find a way in.\n")
            return
        if not merc.IS_TRUSTED(ch, merc.ANGEL) and not merc.IS_SET(portal.value[2], merc.GATE_NOCURSE) \
        ...
        elif IS_SET(portal.value[2], merc.GATE_BUGGY) and (random.randint(1, 99) < 5):
        ...
        or (location.is_private() and not merc.IS_TRUSTED(ch, merc.IMPLEMENTOR)):
