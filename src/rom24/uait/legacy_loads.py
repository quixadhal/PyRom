...
class AREA_SERIALIZER:
    ...
    def to_Pickle(self):
        ...
        for room in self.rooms:
            with open(
                room_save_dir + "/" + str(room.vnum) + settings.PKL_EXTN, "wb"
            ) as rf:
                pickle.dump(room, rf, pickle.HIGHEST_PROTOCOL)
        for mob in self.mobiles:
            with open(
                mobile_save_dir + "/" + str(mob.vnum) + settings.PKL_EXTN, "wb"
            ) as mf:
                pickle.dump(mob, mf, pickle.HIGHEST_PROTOCOL)
        for object in self.objects:
            with open(
                obj_save_dir + "/" + str(object.vnum) + settings.PKL_EXTN, "wb"
            ) as of:
                pickle.dump(object, of, pickle.HIGHEST_PROTOCOL)
        for shop in self.shops:
            with open(
                shop_save_dir + "/" + str(shop.keeper) + settings.PKL_EXTN, "wb"
            ) as sf:
                pickle.dump(shop, sf, pickle.HIGHEST_PROTOCOL)
        for reset in self.resets:
            with open(
                reset_save_dir + "/" + str(reset) + settings.PKL_EXTN, "wb"
            ) as ref:
...
#         os.makedirs(settings.SOCIAL_DIR, exist_ok=True)
...
def to_Pickle():
    for area_serial in world_classes.serializer_list:
        ...
        for area in area_serial.area:
            with open(
                area_save_dir + "/" + area_serial.area_name + ".pickle", "wb"
            ) as af:
                pickle.dump(area, af, pickle.HIGHEST_PROTOCOL)
                af.close()
...
def load_mobiles(area, pArea):
    ...
    while w != "0":
        ...
        mob.start_pos = state_checks.name_lookup(
            tables.position_table, mob.start_pos, "short_name"
        )
        mob.default_pos = state_checks.name_lookup(
            tables.position_table, mob.default_pos, "short_name"
        )
...
def load_objects(area, pArea):
    ...
    while w != "0":
        ...
        if obj.item_type == merc.ITEM_WEAPON:
            ...
            obj.item_type == merc.ITEM_DRINK_CON or obj.item_type == merc.ITEM_FOUNTAIN
        ):
            area, obj.value[0] = game_utils.read_int(area)
            area, obj.value[1] = game_utils.read_int(area)
            ...
            obj.item_type == merc.ITEM_POTION
            or obj.item_type == merc.ITEM_SCROLL
